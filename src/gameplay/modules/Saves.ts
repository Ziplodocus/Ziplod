...
//   let emptyRow = [];
//   let saveData = '{"name": "' + curPlayer.name + '", "health": ' +
//     curPlayer.health + ', "streak": ' + curPlayer.streak + "}";
//   for (let row = 0; row < saveSlots.length; row++) {
//     if (saveSlots[row][0] !== "") {
//       if (
//         curPlayer.name.toLowerCase() ==
//           JSON.parse(saveSlots[row]).name.toLowerCase()
//       ) {
...
//       "Save slots full",
//       "Please type a name of a save slot to delete or press cancel to leave the simulator",
