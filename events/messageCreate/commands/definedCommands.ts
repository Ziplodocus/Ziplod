...
import { MessageButton, MessageActionRow, RoleManager, GuildMemberRoleManager } from "discord.js";
import { extendedMessage } from "../messageCreate.js";
import { client } from "../../../ziplod.js";

export function definedCommands(msg : extendedMessage) {
	return {
		comedy() {
			if (!msg.voiceChannel)
		...
		async roleCall() {
			...
			collector.on('collect', async () => console.log('yee'));
...
function checkValidity(msg : extendedMessage) {
