...
export function definedCommands(extMsg: extendedMessage) {
	...
	return {
		...
		async endRolers() {
			...
			if (!message.member.permissions.has("MANAGE_ROLES", true))
				return message.reply(
					"Your priviledge has been checked, and you have been deemed too pathetic to use this command."
				);
			if (
				!client.guilds.cache
					.get(guildId)
			...
			let replyData = [];
			const roles =
				message.guild.roles || (await roles.fetch(null, { cache: true }));
